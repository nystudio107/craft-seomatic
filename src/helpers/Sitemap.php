...
class Sitemap
{
     ...
     */
    public static function generateSitemap(array $params, string $templateClass)
    {
        /** @var SitemapTemplate $templateClass */
        ...
        $totalElements = null;
        $xmlNs = $templateClass::getXmlNs($metaBundle, $multiSite);

        // Get all the elements for this meta bundle type
        $seoElement = Seomatic::$plugin->seoElements->getSeoElementByMetaBundleType($metaBundle->sourceBundleType);

        if ($seoElement !== null) {
            $totalElements = $templateClass::getTotalElements($metaBundle, $seoElement);
        }
        ...
        $stashedSitemapAttrs = $templateClass::getSiteMapVars($metaBundle)->getAttributes();
        ...
        $paginator = new Paginator($templateClass::getSitemapElementsQuery($seoElement, $metaBundle), [
        ...
        while ($currentElement < $totalElements) {
            ...
            foreach ($elements as $element) {
                ...
                $templateClass::getSiteMapVars($metaBundle)->setAttributes($stashedSitemapAttrs, false);
                $metaBundle->metaGlobalVars->setAttributes($stashedGlobalVarsAttrs, false);

                // Combine in any per-entry type settings
                self::combineEntryTypeSettings($seoElement, $element, $metaBundle, $templateClass);

                // Make sure this entry isn't disabled
                self::combineFieldSettings($element, $metaBundle, $templateClass);
                ...
                if ($path !== null && $templateClass::getIsSitemapEnabled($metaBundle) && $robotsEnabled) {
                    ...
                    if ($multiSite && $templateClass::getSiteMapVars($metaBundle)->sitemapAltLinks) {
                        ...
                        foreach ($metaBundle->sourceAltSiteSettings as $altSiteSettings) {
                            if (\in_array($altSiteSettings['siteId'], $groupSiteIds, false)) {
                                ...
                                if ($altElement && $altElement->url) {
                                    ...
                                    if ($altMetaBundle) {
                                        // Make sure this entry isn't disabled
                                        self::combineFieldSettings($altElement, $altMetaBundle, $templateClass);
                                        $altUrl = UrlHelper::absoluteUrlWithProtocol($altElement->url);
                                        if ($templateClass::getIsSitemapEnabled($altMetaBundle)) {
                    ...
                    $additionalData = $templateClass::getAdditionalDataForElement($metaBundle, $element);

                    $sitemapData .= $templateClass::generateSitemapEntry(
                        Html::encode($url),
                        $dateUpdated->format(\DateTime::W3C),
                        $templateClass::getSiteMapVars($metaBundle)->sitemapChangeFreq,
                        $templateClass::getSiteMapVars($metaBundle)->sitemapPriority,
                        $altLinks,
                        $additionalData
                    );
                }

                $sitemapData .= $templateClass::generateAdditionalEntriesForElement($metaBundle, $element);
            }

            $paginator->currentPage++;
        }

        $sitemap = $templateClass::wrapSitemap($sitemapData, $xmlNs);
        ...
        $cacheKey = $templateClass::CACHE_KEY.$groupId.$type.$handle.$siteId;
        $dependency = new TagDependency([
            'tags' => [
                $templateClass::GLOBAL_SITEMAP_CACHE_TAG,
                $templateClass::SITEMAP_CACHE_TAG . $handle . $siteId,
            ],
        ]);
     ...
     */
    protected static function combineEntryTypeSettings($seoElement, Element $element, MetaBundle $metaBundle, $templateClass)
    {
        /** @var SitemapTemplate $templateClass */
        ...
        if (!empty($seoElement::typeMenuFromHandle($metaBundle->sourceHandle))) {
            ...
            if ($entryTypeBundle) {
                ...
                $attributes = $templateClass::getSiteMapVars($entryTypeBundle)->getAttributes();
                $attributes = array_filter(
                    $attributes,
                    [ArrayHelper::class, 'preserveBools']
                );
                $templateClass::getSiteMapVars($metaBundle)->setAttributes($attributes, false);
     ...
     * @param MetaBundle $metaBundle
     */
    protected static function combineFieldSettings(Element $element, MetaBundle $metaBundle, $templateClass)
    {
        /** @var SitemapTemplate $templateClass */
